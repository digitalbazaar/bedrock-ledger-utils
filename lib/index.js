...
api.dereferenceElectorPool = async ({electorPool, ledgerNode}) => {
  ...
  for(const e of electorPool) {
    ...
    if(typeof e.service === 'string') {
      ...
      const type = continuityServiceType;
      const service = _.find(electorService, {id: e.service, type});
      if(!service) {
        // TODO: fixup error
        throw new BedrockError(
          ...
          'service descriptor.', 'NotFoundError', {
            elector: record,
            expectedService: {id: e.service, type}
          });
      }
...
api.getElectorPoolElectors = async ({ledgerConfiguration, ledgerNode}) => {
  ...
  // FIXME: specify maxBlockHeight? I believe the latest document is always
  // desired/required
